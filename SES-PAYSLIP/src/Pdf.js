...
const API_BASE_URL = "http://localhost:7008/api";
  ...
  // Get the state passed from handleViewPayslip
  const { returnTab, ...payslipData } = location.state || {};

  const handleBack = () => {
    if (returnTab) {
      // Go back to the tab we came from without routing
  ...
  const calculateProratedEarnings = (earningsData, paidDays, totalDays) => {
    ...
    // Calculate prorated amounts for each earnings component
    const proratedBasic =
    ...
    // Performance bonus and arrears are usually fixed amounts, not prorated
  ...
  const fetchEmployeeDetails = async (empId) => {
    try {
      ...
      // Format the date of joining from backend
      const formattedDateOfJoining = formatDateForDisplay(
        response.data.date_of_joining,
      );

      // Update editedData with the fetched date of joining
      setEditedData((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    const initializeData = async () => {
      if (data) {
        ...
        // Always fetch employee details to get the correct date of joining
        // This ensures we always have the latest date from the database
        const formattedDate = await fetchEmployeeDetails(data.id);

        // Update the editedData with the fetched date if available
        if (formattedDate) {
          setEditedData((prev) => ({
            ...prev,
  ...
  // Function to check if PF is applicable for the employee
  const checkPfApplicability = (employeeData) => {
    // Check if pf_applicable is explicitly set to 0/false
    if (
    ...
    // If either employee PF or employer PF is greater than 0, consider PF applicable
  ...
  const fetchEarningsData = async (empId) => {
    try {
      ...
      // Update PF applicability based on fetched earnings data
      if (response.data) {
      ...
      setFetchError(true);
      // Set default values based on passed data with proper number conversion
      setEarnings({
  ...
  // Calculate per day rate for display
  ...
  const perDayRate =
    attendanceInfo.totalDays > 0
      ? totalMonthlyEarnings / attendanceInfo.totalDays
      : 0;

  // Use prorated earnings for total
  const totalEarnings = proratedEarnings.total_earnings;

  // Deductions (these are typically fixed, not prorated)
  ...
  const handleSave = async () => {
    // Validate date of joining format if it's not empty and not "Not Available"
    if (
    ...
    try {
      ...
      // Convert date back to YYYY-MM-DD for API (only if it's a valid date)
      ...
      const response = await axios.post(`${API_BASE_URL}/payslips/generate`, {
        ...
        // Include PF amount only if applicable (but not shown in deductions)
