...
function PayslipApp() {
  ...
  const [employeeForm, setEmployeeForm] = useState({
    emp_id: "", name: "", designation: "", date_of_joining: "", PAN: "",
    basic_salary: "", house_rent_allowence: "", transport_allowance: "",
    internet_allowance: "", medical_allowance: "", professional_tax: "",
    bank_account_number: "", IFSC_code: "", bank_name: "", pf_applicable: true,
  });

  const [reportFilters, setReportFilters] = useState({
    month: new Date().getMonth() + 1, year: new Date().getFullYear(), emp_id: "",
  });
  ...
  const convertToDBFormat = (ds) => {
    ...
    if (/^\d{2}\/\d{2}\/\d{4}$/.test(ds)) {
      const [d,m,y] = ds.split("/"); return `${y}-${m}-${d}`;
    }
    return ds;
  };
...
const fetchEmployees = useCallback(async () => {
  ...
  const resetEmployeeForm = () => {
    setEmployeeForm({ emp_id:"",name:"",designation:"",date_of_joining:"",PAN:"",
      basic_salary:"",house_rent_allowence:"",transport_allowance:"",internet_allowance:"",
      medical_allowance:"",professional_tax:"",bank_account_number:"",IFSC_code:"",bank_name:"",pf_applicable:true });
  ...
  const handleEmployeeInputChange = (e) => {
    ...
    if (name === "date_of_joining") {
      ...
      setEmployeeForm({ ...employeeForm, [name]: type==="checkbox" ? checked : value });
    }
  };

  const handleEmployeeSubmit = async (e) => {
    e.preventDefault();
    if (!validateEmployeeForm()) { showMessage("error","Please fix form errors"); return; }
    if (!validateDateFormat(employeeForm.date_of_joining)) { showMessage("error","Please enter valid date DD/MM/YYYY"); return; }
    setLoading(true);
    try {
    ...
    } catch(err) { showMessage("error", err.response?.data?.error || "Error saving employee"); }
    setLoading(false);
  };

  const editEmployee = (emp) => { setEmployeeForm(emp); setEditingEmployee(emp); setActiveTab("employees"); };
  ...
  const downloadEmployeeDetailsExcel = async () => {
    ...
    try {
      ...
      link.setAttribute("download", fn); document.body.appendChild(link); link.click(); link.remove();
      window.URL.revokeObjectURL(url);
      showMessage("success","BLKPAY Excel downloaded");
    } catch { showMessage("error","Error downloading Excel"); }
    setLoading(false);
  };
...
const handleViewPayslip = (payslip) => {
  ...
  const handleEditPayslip = (payslip) => {
    const editData = {
      ...
      salary_year: parseInt(payslip.salary_year), advance_salary: parseFloat(payslip.advance_salary)||0,
      paid_days: parseInt(payslip.paid_days)||0, holidays: parseInt(payslip.holidays)||0,
      leaves: parseInt(payslip.leaves)||0, gross_salary: parseFloat(payslip.gross_salary)||0,
      pf_deduction: parseFloat(payslip.pf_deduction)||0,
      professional_tax_deduction: parseFloat(payslip.professional_tax_deduction)||0,
      performance_bonus: parseFloat(payslip.performance_bonus)||0, arrears: parseFloat(payslip.arrears)||0,
      total_deductions: parseFloat(payslip.total_deductions)||0, net_salary: parseFloat(payslip.net_salary)||0,
      payslip_id: payslip.id, name: payslip.name, designation: payslip.designation,
      bank_account_number: payslip.bank_account_number, IFSC_code: payslip.IFSC_code,
      bank_name: payslip.bank_name, PAN: payslip.PAN, pf_applicable: payslip.pf_applicable,
    };
  ...
  const TopNav = () => (
    <nav className="navbar navbar-expand-md mb-3 px-3"
      style={{ background:"#0f263f", borderRadius:"6px" }}>
      <span className="navbar-brand text-white fw-bold me-4" style={{fontSize:"1rem"}}>
        Payslip Management
      </span>
      <button className="navbar-toggler border-0" type="button"
        data-bs-toggle="collapse" data-bs-target="#payslipNav"
            ...
            return (
              <li className="nav-item" key={tab.key}>
                <button
                  ...
                  style={{ background: activeTab===tab.key ? "rgba(255,255,255,0.2)" : "transparent",
  ...
  return (
    <div>
      ...
      {message?.text && (
        <div className={`alert ${message.type==="error"?"alert-danger":"alert-success"} py-2 mb-3`}>
          {message.text}
        </div>
      )}
        ...
        <Container fluid className="py-3">
          <Form onSubmit={handleEmployeeSubmit}>
            <Card className="mb-4 shadow">
              <Card.Header className="text-center fw-bold bg-info text-white">Employee Details</Card.Header>
              <Card.Body>
                <Row className="g-3">
                  <Col lg={4} md={6} sm={12}>
                  ...
                  <Col lg={4} md={6} sm={12}>
                    ...
                    <Form.Control name="name" value={employeeForm.name} onChange={handleEmployeeInputChange} required />
                  </Col>
                  <Col lg={4} md={6} sm={12}>
                    <Form.Label>Designation *</Form.Label>
                    <Form.Control name="designation" value={employeeForm.designation} onChange={handleEmployeeInputChange} required />
                  </Col>
                  <Col lg={4} md={6} sm={12}>
                    <Form.Label>Date of Joining *</Form.Label>
                    <Form.Control name="date_of_joining" value={employeeForm.date_of_joining} onChange={handleEmployeeInputChange} placeholder="DD/MM/YYYY" required />
                  </Col>
                  <Col lg={4} md={6} sm={12}>
                    <Form.Label>PAN *</Form.Label>
                    <Form.Control name="PAN" value={employeeForm.PAN} onChange={handleEmployeeInputChange} required />
            ...
            <Card className="mb-4 shadow">
              <Card.Header className="text-center fw-bold bg-success text-white">Salary Details</Card.Header>
              <Card.Body>
                <Row className="g-3 align-items-end">
                  <Col lg={4} md={6} sm={12}>
                    <Form.Label>Basic Salary *</Form.Label>
                    <Form.Control type="number" name="basic_salary" value={employeeForm.basic_salary} onChange={handleEmployeeInputChange} required />
                  </Col>
                  <Col lg={4} md={6} sm={12}>
                    <Form.Check type="checkbox" name="pf_applicable" checked={employeeForm.pf_applicable} onChange={handleEmployeeInputChange} label="PF Applicable (12%)" className="mt-4" />
                  </Col>
                  <Col lg={4} md={6} sm={12}>
                    <Form.Label>PF Amount</Form.Label>
                    <Form.Control value={employeeForm.pf_applicable && employeeForm.basic_salary ? `₹ ${calculatePF(employeeForm.basic_salary,true).toFixed(2)}` : "₹ 0"} readOnly className="bg-light fw-bold" />
                  </Col>
                  {["house_rent_allowence","transport_allowance","internet_allowance","medical_allowance","professional_tax"].map(field => (
                    <Col lg={4} md={6} sm={12} key={field}>
                      <Form.Label>{field.replace(/_/g," ").replace(/\b\w/g,c=>c.toUpperCase())}</Form.Label>
                      <Form.Control type="number" name={field} value={employeeForm[field]} onChange={handleEmployeeInputChange} />
                    </Col>
                  ))}
                </Row>
              </Card.Body>
            ...
            <Card className="mb-4 shadow">
              <Card.Header className="text-center fw-bold bg-secondary text-white">Bank Details</Card.Header>
              <Card.Body>
                <Row className="g-3">
                  <Col lg={4} md={6} sm={12}>
                    <Form.Label>Account Number *</Form.Label>
                    <Form.Control name="bank_account_number" value={employeeForm.bank_account_number} onChange={handleEmployeeInputChange} required />
                    {errors.bank_account_number && <small className="text-danger">{errors.bank_account_number}</small>}
                  </Col>
                  <Col lg={4} md={6} sm={12}>
                    <Form.Label>IFSC *</Form.Label>
                    <Form.Control name="IFSC_code" value={employeeForm.IFSC_code} onChange={handleEmployeeInputChange} required />
                    {errors.IFSC_code && <small className="text-danger">{errors.IFSC_code}</small>}
                  </Col>
                  <Col lg={4} md={6} sm={12}>
                    <Form.Label>Bank Name *</Form.Label>
                    <Form.Control name="bank_name" value={employeeForm.bank_name} onChange={handleEmployeeInputChange} required />
                  </Col>
                </Row>
              </Card.Body>
            </Card>
            ...
            <div className="d-flex justify-content-between mt-3">
              <Button variant="secondary" onClick={() => setActiveTab("employeelist")}>Back</Button>
              <Button type="submit" disabled={loading} style={{ backgroundColor:"#6c5ce7", color:"#fff", border:"none" }}>
        ...
        <div className="container-fluid py-3 px-4">
          ...
          <div className="row g-3 align-items-end mb-4">
            <div className="col-md-3">
              ...
              <select name="month" className="form-select" value={reportFilters.month}
            ...
            <div className="col-md-3">
              ...
              <select name="year" className="form-select" value={reportFilters.year}
                onChange={e => setReportFilters({...reportFilters, year: parseInt(e.target.value)})}>
                {years.map(y => <option key={y} value={y}>{y}</option>)}
              </select>
            </div>
            <div className="col-md-3">
              <button onClick={fetchPayslips} disabled={loading} className="btn btn-primary w-100">
          ...
          {payslips.length > 0 ? (
            <div className="table-responsive">
              <h4 className="mb-3 text-center" style={{color:"#0d5aa7"}}>
                Payslips For {months.find(m=>m.value===reportFilters.month)?.label} {reportFilters.year}
              </h4>
              <table className="table table-bordered table-hover align-middle text-center">
                <thead className="table-info">
                  <tr><th>S.No</th><th>Emp ID</th><th>Emp Name</th><th>Salary (₹)</th><th>Bank Account</th><th>IFSC</th><th>Options</th></tr>
                </thead>
                <tbody>
                  {payslips.map((p,i) => (
                    <tr key={p.id}>
                      <td>{i+1}</td><td>{p.emp_id}</td><td>{p.name}</td>
                      <td>₹{Number(p.net_salary||0).toFixed(2)}</td>
                      <td>{p.bank_account_number||"-"}</td><td>{p.IFSC_code||"-"}</td>
                      <td>
                        <div className="d-flex justify-content-center gap-2">
                          <button onClick={()=>handleViewPayslip(p)} className="action-btn view-btn"><FaEye className="me-1"/>View</button>
                          <button onClick={()=>handleEditPayslip(p)} className="action-btn edit-btn"><FaEdit className="me-1"/>Edit</button>
                          <button onClick={()=>deletePayslip(p.id)} className="action-btn delete-btn"><FaTrash className="me-1"/>Delete</button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="alert alert-info text-center">No payslips found for the selected period.</div>
          ) : null}
        </div>
      )}
            ...
            <div className="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3">
              ...
              <div className="d-flex gap-3 align-items-end flex-wrap">
                <div>
                  ...
                  <select className="form-select" value={blkpayFilters.month}
                    onChange={e => setBlkpayFilters({...blkpayFilters, month:parseInt(e.target.value)})}>
                    {months.map(m=><option key={m.value} value={m.value}>{m.label}</option>)}
                  </select>
                </div>
                <div>
                  <label className="form-label mb-1">Year</label>
                  <select className="form-select" value={blkpayFilters.year}
                    onChange={e => setBlkpayFilters({...blkpayFilters, year:parseInt(e.target.value)})}>
                    {years.map(y=><option key={y} value={y}>{y}</option>)}
                  </select>
                </div>
                <button onClick={downloadEmployeeDetailsExcel} className="btn btn-dark" disabled={loading}>
                  {loading ? "Generating..." : "Download BLKPAY Excel"}
                </button>
              </div>
            </div>
            <p className="text-muted mb-1">Download employee payment data in BLKPAY format for the selected month and year.</p>
            <p className="text-muted small">Note: Only payslips generated for {months.find(m=>m.value===blkpayFilters.month)?.label} {blkpayFilters.year} will be included.</p>
          </div>
          <div className="d-flex gap-3 flex-wrap mb-3">
            <div>
              <label className="form-label">Month</label>
              <select className="form-select" value={reportFilters.month}
                onChange={e=>setReportFilters({...reportFilters,month:parseInt(e.target.value)})}>
                {months.map(m=><option key={m.value} value={m.value}>{m.label}</option>)}
              </select>
            </div>
            <div>
              <label className="form-label">Year</label>
              <select className="form-select" value={reportFilters.year}
                onChange={e=>setReportFilters({...reportFilters,year:parseInt(e.target.value)})}>
                {years.map(y=><option key={y} value={y}>{y}</option>)}
              </select>
            </div>
            ...
            <div className="d-flex gap-2 align-self-end">
              <button className="btn btn-primary" onClick={fetchMonthlyReport}>Monthly Report</button>
              <button className="btn btn-secondary" onClick={fetchYearlyReport}>Yearly Report</button>
              ...
              {loading ? <p className="text-center">Loading...</p> : (
                <div className="table-responsive">
                  <table className="table table-bordered table-hover align-middle text-center">
                    <thead className="table-info">
                      <tr>
                        <th>S.No</th><th>Emp ID</th><th>Name</th><th>Designation</th>
                        <th>Date of Joining</th><th>PAN</th><th>Gross (₹)</th>
                        <th>PF</th><th>Bank A/c</th><th>Bank Name</th><th>IFSC</th><th>Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {Array.isArray(employees) && employees.map((emp,i) => {
                        ...
                        return (
                          <tr key={emp.emp_id||i} className={i%2===0?"table-light":""}>
                            <th scope="row">{i+1}</th>
                            <td>{emp.emp_id||"-"}</td><td>{emp.name||"-"}</td>
                            <td>{emp.designation||"-"}</td><td>{emp.date_of_joining||"-"}</td>
                            <td>{emp.PAN}</td>
                            <td className="fw-semibold text-primary">₹{gross.toFixed(2)}</td>
                            <td><span className={`badge ${emp.pf_applicable?"bg-success":"bg-danger"}`}>{emp.pf_applicable?"Yes":"No"}</span></td>
                            <td>{emp.bank_account_number||"-"}</td>
                            <td>{emp.bank_name||"-"}</td><td>{emp.IFSC_code||"-"}</td>
                            <td>
                              <div className="d-flex justify-content-center gap-1 flex-wrap">
                                <button className="btn btn-sm" style={{backgroundColor:"#2d7dce",color:"#fff",borderRadius:"8px"}}
                                  onClick={()=>{ setSelectedEmployee(emp); setActiveTab("readEmployee"); }}><FaEye/> View</button>
                                <button className="btn btn-sm" style={{backgroundColor:"#8dc02f",color:"#fff",borderRadius:"8px"}}
                                  onClick={()=>editEmployee(emp)}><FaEdit/> Edit</button>
                                <button className="btn btn-sm btn-danger" style={{borderRadius:"8px"}}
                                  onClick={()=>deleteEmployee(emp.emp_id)}><FaTrash/> Delete</button>
                              </div>
                            </td>
                          </tr>
                        );
